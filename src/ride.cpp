...
//!!!!!
std::vector<Block> ride::blockchain;
std::vector<ride *> ride::allNodes;
// TODO: why do we have an allNodes vector in the ride class?
// if I am not mistaken this should be in the blockchain class or sth
int ride::numClusterHeads = 3;
int ride::globalBlockId = 0;
...
// really necessary
std::set<int> ride::maliciousNodeIds;
int ride::totalBadServicesReceived = 0;
int ride::totalBenevolentNodes = 0;
...
void ride::setMalicious(AttackerType type) {
...
// ************************************************************
void ride::populateServiceTable() {
  EV << "Node " << getId()
  ...
  for (int i = 0; i < gateSize("inoutGate"); i++) {
    ...
    if (outGate->isConnected()) {
      cModule *connectedModule = outGate->getNextGate()->getOwnerModule();
      ride *neighborNode = dynamic_cast<ride *>(connectedModule);
      if (neighborNode) {
...
void ride::handleServiceRequestMsg(cMessage *msg) {
...
ride *ride::getNodeById(int targetId) {
...
void ride::handleServiceResponseMsg(cMessage *msg) {
...
void ride::handleFinalServiceRequestMsg(cMessage *msg) {
...
void ride::handleFinalServiceResponseMsg(cMessage *msg) {
...
void ride::handleServiceRatingMsg(cMessage *msg) {
...
void ride::handleNetworkMessage(cMessage *msg) {
...
void ride::handleMessage(cMessage *msg) {
...
void ride::handleServiceRequest(int requesterId) {
...
double ride::calcQuality(const double potency, const double consistency) {
  enum AttackerType type = this->attackerType;
  switch (type) {
  case BENEVOLENT:
...
bool ride::givesService(std::string serviceType) {
 ...
 */
double ride::calculateRarity(std::string serviceType) {
  int howManyNodes = 0;
...
void ride::sendRating(int providerId, double rating) {
...
void ride::sendTransactionToClusterHead(ServiceRating *transaction) {
  ...
  ride *bestClusterHead = clusterHeads[randomIndex];
...
double ride::calculateIndirectTrust(int reqId, int provId, double time) {
  // WARN: This works for two layers only for now...

  // we know that i does not know j yet, get those who i knows:
  std::vector<ride *> nodesKnownByRequestor;
  for (ride *node : allNodes) {
  ...
  for (ride *node : nodesKnownByRequestor) {
    int nodeId = node->getId();
...
bool ride::enoughInteractions(int requestorId, int providerId) {
 ...
 */
double ride::calculateDecay(double currentTime, double blockTime) {
