...
static std::uniform_real_distribution<double> uniform_real_dist{ 0, 1 }; // bu da burda dursun madem
...
// * a map where map[i] gives the GT_i
std::map<int, double> VoteAgg::globalTrustScores;
// * current ordering of nodes where v[0] is the (globally) most trusted node
...
bool
VoteAgg::noMalDominatedClusters ()
{
    int clusterSize = getParentModule ()->par ("clusterSize"); // we calculate this, not set
    ...
    for (int clusterIndex = 0; clusterIndex < clusterCount; clusterIndex++)
	{
	    ...
	    for (int nodeIndex = 0; nodeIndex < clusterSize; nodeIndex++)
		{
		    ...
		    int nodeId = (clusterIndex * clusterSize) + nodeIndex + 2; // ids start from 2
...
void
VoteAgg::setMalicious (AttackerType type)
{
...
void
VoteAgg::setPotencyAndConsistency ()
{
    int id = getId ();
...
VoteAgg::initialize ()
{
    ...
    if (hasPar ("aggregationMethod"))
	{
		...
		case 2:
		    ...
		    break;
		case 3:
		    aggregationMethod = AGG_APPROVAL;
		    break;
		case 4:
...
static void
printBlockChain (std::vector<Block> blockchain)
{
...
void
VoteAgg::populateServiceTable ()
{
    EV << "Node " << getId () << " is now filling its service table with direct neighbors..."
       << endl;
    for (int i = 0; i < gateSize ("inoutGate"); i++)
	{
	    cGate *outGate = gate ("inoutGate$o", i);
...
void
VoteAgg::handleServiceRequestMsg (cMessage *msg)
{
	...
	{
	    EV << "Node " << getId () << " does NOT provide requested service: " << requestedService
	       << ". Ignoring." << endl;
	    delete request;
	    return;
	}

    EV << "Node " << getId () << " will respond to service request from Node " << requesterId
       << endl;
...
VoteAgg *
VoteAgg::getNodeById (int targetId)
{
...
auto
VoteAgg::randomPairOfMapping (const std::map<int, double> &mapping)
{
    if (mapping.empty ())
...
VoteAgg::epsilonGreedyMaxPair (const std::map<int, double> &mapping)
{
...
void
VoteAgg::handleServiceResponseMsg (cMessage *msg)
{
	    ...
	    if (pendingResponses.empty ())
		{
		    ...
		    if (!respondedProviders.empty ())
			{
			    ...
			    for (const auto &entry : respondedProviders)
				{
				    ...
				    mergedProviders[candidateId] = mergeTrustScore (
					candidateId); // bu kısım yeni eklendi merge için
				}
...
void
VoteAgg::handleFinalServiceRequestMsg (cMessage *msg)
{
...
double
VoteAgg::updateMyRating (int providerId, double rating)
{
...
void
VoteAgg::handleFinalServiceResponseMsg (cMessage *msg)
{
    ...
    double rating = calculateRating (quality, timeliness, rarity); // handles attacks too
    ...
    EV << "Direct trust update: requester=" << getId () << " provider=" << providerId
       << " rating=" << rating << " positiveSum=" << alterandum.sumOfPositiveRatings
       << " absSum=" << alterandum.sumOfAllRatings << " directTrust=" << alterandum.value ()
       << "\n";
    delete response;
}
 ...
 */
void
VoteAgg::propagateRatingReplica (ServiceRating *transaction)
{
...
int
VoteAgg::addBlockToBC (double rating, int requesterId, int providerId)
{
    int blockId = ++globalBlockId;
    Block newBlock = { .blockId = blockId,
		       .validatorId = getId (),
		       .transactionData = "Rating: " + std::to_string (rating) + " from "
					  + std::to_string (requesterId) + " to "
...
void
VoteAgg::handleServiceRatingMsg (cMessage *msg)
{
...
void
VoteAgg::handleNetworkMessage (cMessage *msg)
{
...
void
VoteAgg::handleSelfMessage (cMessage *msg)
{
    const char *msgName = msg->getName ();
	...
	{
	    EV << "HYBRID node " << getId () << " has switched: will now give BAD SERVICES.\n";
    ...
    else if (strcmp (msgName, "globalTrustUpdate") == 0)
	{
	    ...
	    recordScalar (
		("GlobalTrustListUpdatedAt_" + std::to_string ((int)simTime ().dbl ())).c_str (),
		1);
    ...
    else if (strcmp (msg->getName (), "badServiceLogger") == 0)
	{
	    ...
	    if (it != trustMap.end ())
		{
		    ...
		    recordScalar (
			("OpportunisticNodeTrustScoreAt_" + std::to_string ((int)simTime ().dbl ()))
			    .c_str (),
...
void
VoteAgg::handleMessage (cMessage *msg)
{
...
VoteAgg::electClusterHeads ()
{
	...
	{ // bunu da daha farklı yazabiliriz böyle biraz saçma oldu ama
...
void
VoteAgg::initiateServiceRequest ()
{
    ...
    for (int providerId : providerIds)
	{
	    ...
	    EV << "Sent service request to Node " << providerId << " for type " << SERVICE_TYPE
	       << endl;
	}
}

void
VoteAgg::handleServiceRequest (int requesterId)
{
...
bool
VoteAgg::performsCamouflage (double camouflageRate)
{
 ...
 */
double
VoteAgg::calculateRatingCamouflage (double quality, double timeliness, double rarity)
{
    if (performsCamouflage (this->camouflageRate))
	{ // normal rating
...
double
VoteAgg::calcQualityCamouflage (double potency, double consistency)
{
    if (performsCamouflage (this->camouflageRate))
	{ // normal service
...
[[deprecated ("WE HAVE NOT IMPLEMENTED THE badMouthingRating METHOD PROPERLY YET!!!!")]]
double
VoteAgg::badMouthingRating ()
{
    return 0;
}
double
...
double
VoteAgg::calculateRating (double quality, double timeliness, double rarity)
{
	...
	case HYBRID:
		...
		if (!provider)
		    return 0;
		return provider->benevolent ? calculateRatingBenevolent (-10, timeliness, rarity)
					    : calculateRatingBenevolent (8, timeliness, rarity);
	    } // punish honest, reward malicious
	case OPPORTUNISTIC:
	    return calculateRatingBenevolent (quality, timeliness, rarity);
	case COLLABORATIVE:
	    {
		...
		return provider->benevolent
			   ...
			   : calculateRatingBenevolent (8, timeliness,
							rarity); // punish honest, reward malicious
	    }
	default:
...
double
VoteAgg::calcQuality (const double potency, const double consistency)
{
    enum AttackerType type = this->attackerType;
    switch (type)
	{
	...
	case BAD_SERVICE_GOOD_RATING:
	    return -10;
	case HYBRID:
	    return hybridHasSwitched ? -10
				     : calcQualityBenevolent (
					   potency, consistency); // before switch: good service
 ...
 */
double
VoteAgg::calcQualityBenevolent (const double potency, const double consistency)
{
    double quality;
 ...
 */
double
VoteAgg::calculateRatingBenevolent (double quality, double timeliness, double rarity)
{
    double rating;
...
void
VoteAgg::sendRating (int providerId, double rating)
{
    // rating in (-10,10)'e karar verildi
    EV << "IoT Node " << getId () << " gives a score of " << rating << " to Node " << providerId
       << endl;
...
void
VoteAgg::sendTransactionToClusterHead (ServiceRating *transaction)
{
...
bool
VoteAgg::enoughInteractions (int requestorId, int providerId)
{
 ...
 */
double
VoteAgg::calculateDecay (double currentTime, double blockTime)
{
 ...
 */
bool
VoteAgg::extract (const std::string &input, double &rating, int &requesterId, int &providerId)
{
    ...
    if ((iss >> tmp >> rating) && (iss >> tmp >> requesterId) && (iss >> tmp >> providerId))
	{
	    return true;
	}
    else
...
*/
std::vector<int>
VoteAgg::sortNodesByScore (const std::map<int, double> &scores)
{
...
VoteAgg::DirectTrustMatrix
VoteAgg::buildDirectTrustMatrix ()
{
...
// TODO we could use template parameters instead of switch case?
// Would be slightly faster and more sound.
void
VoteAgg::updateGlobalTrustList ()
{
    if (allNodes.empty ())
	return;
...
void
VoteAgg::updateGlobalTrustAdditive ()
{
...
void
VoteAgg::updateGlobalTrustMultiplicative ()
{
    globalTrustScores.clear ();
    DirectTrustMatrix matrix = buildDirectTrustMatrix ();
...
void
VoteAgg::updateGlobalTrustBorda ()
{
    globalTrustScores.clear ();
...
/*
 * * Alters the "globalTrustScores" with the Approval method.
 * * Node i gives "1" to j if TS_ij > theta.
 * Otherwise, i gives j "0"
 *
 */
void
VoteAgg::updateGlobalTrustApproval ()
{
    // We do not accumulate the votes.
    // But start anew for each voting.
    globalTrustScores.clear ();
    // DirectTrustMatrix matrix = buildDirectTrustMatrix ();

    for (VoteAgg *target : allNodes)
	{ // starting the voting for "target"
	    int targetId = target->getId ();
	    int votes = 0;

	    for (VoteAgg *voter : allNodes)
		{ // each "other" node will have a vote
		    int voterId = voter->getId ();
		    if (targetId == voterId)
			continue; // cannot vote for itself

		    double votersTrustInTarget = voter->trustMap[targetId].value ();
		    // FIXME I want to see this
		    // assert (votersTrustInTarget == matrix[voterId][targetId]);

		    if (votersTrustInTarget
			>= voter->approval_threshold) // voter trusts the target enough
			votes++;
		}
	    globalTrustScores[targetId] = votes;
	}
}
...
VoteAgg::updateGlobalTrustRelu ()
{
	...
	{ // starting the voting for "target"
		...
		{ // each "other" node will have a vote
			...
			{		// voter trusts the target enough
			    votes += 1; // gives a full vote
			}
		    else
			{ // voter does not trust enough
			  // gives a vote in [0,1)
...
int
VoteAgg::getRankPointFromGlobalOrdering (int nodeId, const std::vector<int> &candidates)
{
	...
	{
	    if (std::find (candidates.begin (), candidates.end (), rankedId) != candidates.end ())
		{
...
double
VoteAgg::mergeTrustScore (int candidateId)
{
...
VoteAgg::finish ()
{
    ...
    for (auto &entry : trustMap)
	{
		...
		= "TrustOf_" + std::to_string (getId ()) + "_in_" + std::to_string (targetId);
