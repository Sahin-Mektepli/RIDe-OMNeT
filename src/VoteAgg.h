...
struct trustScore
{
	...
	if (sumOfAllRatings <= 0.0)
	    return 0.0; // neutral default until enough ratings exist
	return sumOfPositiveRatings / sumOfAllRatings;
    }
};

struct myRatings
{ // now we do not need this because ı store DT in trustmap and trustScore
...
class VoteAgg : public cSimpleModule
{
  public:
    ...
    // FIXME
    // We should probably use proper multi-dimensional arrays instead of these
    // Also, this just serves as a wrapper of trustMap.
  ...
  protected:
    ...
    // === HYPERPARAMETERS FOR AGGREGATION METHODS ===
  ...
  protected:
    ...
    epsilonGreedyMaxPair (const std::map<int, double> &mapping);
    ...
    // There are to fix a linking error.
    // We can either have *everything* as member functions or declare them static.
