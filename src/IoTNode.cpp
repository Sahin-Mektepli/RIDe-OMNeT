...
bool
IoTNode::noMalDominatedClusters ()
{
...
IoTNode::setMalicious (AttackerType type)
{
    ...
    else if (maliciousNodeIds.count (getId ()) > 0)
	{
	    ...
	    //! olabilir
	    attackerType = (type == OPPORTUNISTIC)
			       ? CAMOUFLAGE
...
double
IoTNode::calculateRatingSimilarityCoefficient (int providerId, double newRating)
{
    ...
    EV << "my current rating to " << providerId << " is " << currentRating << "\nnewRating is "
       << newRating << '\n';
    EV << "diff between my rating and current rating is " << diff << " therefore alpha is "
       << 1 / denom << '\n';
...
void
IoTNode::handleServiceRequestMsg (cMessage *msg)
{
...
IoTNode *
IoTNode::getNodeById (int targetId)
{
...
auto
randomPairOfMapping (const std::map<int, double> &mapping)
{
    if (mapping.empty ())
...
auto
IoTNode::epsilonGreedyMaxPair (const std::map<int, double> &mapping)
{
...
void
IoTNode::handleServiceResponseMsg (cMessage *msg)
{
...
double
IoTNode::updateMyRating (int providerId, double rating)
{
...
void
IoTNode::handleFinalServiceResponseMsg (cMessage *msg)
{
 ...
 */
void
IoTNode::propagateRatingReplica (ServiceRating *transaction)
{
...
int
IoTNode::addBlockToBC (double rating, int requesterId, int providerId)
{
...
double
IoTNode::updateTrustScore (int providerId, double rating, double alpha)
{
    if (alpha < 0 || alpha > 1)
	{
    ...
    // test için
    EV << "\nupdateTrustScore for " << providerId << ": rating=" << rating << ", alpha=" << alpha
       << ", effect=" << ratingEffect
...
void
IoTNode::handleServiceRatingMsg (cMessage *msg)
{
...
void
IoTNode::handleNetworkMessage (cMessage *msg)
{
...
void
IoTNode::handleSelfMessage (cMessage *msg)
{
    const char *msgName = msg->getName ();
...
void
IoTNode::handleMessage (cMessage *msg)
{
...
void
IoTNode::handleServiceRequest (int requesterId)
{
...
bool
performsCamouflage (double camouflageRate)
{
 ...
 */
double
IoTNode::calculateRatingCamouflage (double quality, double timeliness, double rarity)
{
...
double
IoTNode::calcQualityCamouflage (double potency, double consistency)
{
    if (performsCamouflage (this->camouflageRate))
	{ // normal service
...
double
IoTNode::calculateRating (double quality, double timeliness, double rarity)
{
...
double
IoTNode::calcQuality (const double potency, const double consistency)
{
    enum AttackerType type = this->attackerType;
    switch (type)
	{
	...
	case OPPORTUNISTIC:
	    return calcQualityBenevolent (potency, consistency);
	case HYBRID:
	    return hybridHasSwitched ? -10 // after switch: bad service
 ...
 */
double
IoTNode::calcQualityBenevolent (const double potency, const double consistency)
{
    double quality;
 ...
 */
double
IoTNode::calculateRatingBenevolent (double quality, double timeliness = 10, double rarity = 10)
{
    double rating;
...
void
IoTNode::sendRating (int providerId, double rating)
{
...
void
IoTNode::sendTransactionToClusterHead (ServiceRating *transaction)
{
...
bool
IoTNode::enoughInteractions (int requestorId, int providerId)
{
 ...
 */
double
IoTNode::calculateDecay (double currentTime, double blockTime)
{
 ...
 */
bool
IoTNode::extract (const std::string &input, double &rating, int &requesterId, int &providerId)
{
